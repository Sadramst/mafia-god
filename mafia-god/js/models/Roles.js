...
export class Roles {
  ...
  static ALL = {
    ...
    negotiator: new Role({
      ...
      team: Team.MAFIA,
      icon: '🤝',
      descriptionEn: 'Mafia member. When alive mafia count reaches threshold (default 2, configurable) and negotiator is alive, negotiator wakes up and selects a target to recruit. If target is simple citizen or suspect → joins mafia team. Otherwise negotiation fails. On a negotiate night, mafia loses their regular shot.',
      descriptionFa: 'عضو تیم مافیا. وقتی تعداد اعضای زنده مافیا به حد مشخصی برسد (پیش‌فرض ۲، قابل تنظیم) و مذاکره‌کننده زنده باشد، مذاکره‌کننده بیدار شده و هدف انتخاب می‌کند. اگر هدف شهروند ساده یا مظنون باشد → به تیم مافیا اضافه می‌شود. در غیر این صورت مذاکره شکست. در شب مذاکره، مافیا شلیک ندارد.',
      nightAction: NightAction.NEGOTIATE,
      nightOrder: 2,
      maxCount: 1,
      unique: true,
    }),
    ...
    jack: new Role({
      ...
      icon: '🔪',
      descriptionEn: 'Independent role. Each night curses one person. If the cursed person is killed or voted out (night or day), Jack is also eliminated. Jack can also be killed by correct salakhi. Jack is immune to all other forms of death (shoot, vote, morning shots, last action cards, etc.). Cannot curse the same person again unless no other option. If shot in day → survives but curse is permanently locked. If targeted by vote → survives but curse is permanently locked. If Citizen Kane reveals Jack → curse is permanently locked and God announces it. When curse is locked, if the cursed player dies Jack is immediately eliminated. Wins if all mafia die or if Jack reaches Chaos (3 players).',
      descriptionFa: 'نقش مستقل. هر شب روی یک نفر طلسم می‌گذارد. اگر فرد طلسم‌شده کشته شود (شب یا روز)، جک هم حذف می‌شود. جک با سلاخی صحیح هم قابل حذف است. جک از تمام انواع دیگر مرگ مصون است (شلیک، رأی، تیر صبحگاهی، کارت حرکت آخر و غیره). نمی‌تواند روی یک نفر دوباره طلسم بگذارد مگر گزینه دیگری نباشد. اگر در روز تیر بخورد → زنده می‌ماند ولی طلسم برای همیشه ثابت می‌شود. اگر هدف رأی قرار بگیرد → زنده می‌ماند ولی طلسم ثابت می‌شود. اگر همشهری کین جک را افشا کند → طلسم ثابت می‌شود و خدا اعلام می‌کند. وقتی طلسم ثابت شود، اگر فرد طلسم‌شده بمیرد جک فوراً حذف می‌شود. برنده می‌شود اگر تمام مافیا بمیرند یا جک به آشوب (۳ نفر) برسد.',
