...
export class Roles {
  ...
  static ALL = {
    ...
    drLecter: {
      ...
      team: 'mafia',
      icon: '💉',
      description: 'دکتر مافیا. هر شب یکی از اعضای مافیا (یا خودش) را نجات می‌دهد. می‌تواند هر فرد دیگر را بدون محدودیت نجات دهد اما خودش را فقط تعداد محدود (پیش‌فرض ۲ بار، قابل تنظیم). هیل تا صبح فعال است — اگر کسی شلیک کند هیل جذب شده و فرد زنده می‌ماند.',
    ...
    drWatson: {
      ...
      team: 'citizen',
      icon: '⚕️',
      description: 'هر شب یک نفر (یا خودش) را نجات می‌دهد. می‌تواند هر فرد دیگر را بدون محدودیت نجات دهد اما خودش را فقط تعداد محدود (پیش‌فرض ۲ بار، قابل تنظیم). هیل تا صبح فعال است — اگر کسی شلیک کند هیل جذب شده و فرد زنده می‌ماند.',
      nightAction: 'heal',
      nightOrder: 10,
      maxCount: 1,
      unique: true,
