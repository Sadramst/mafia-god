...
export class Roles {
  ...
  static ALL = {
    ...
    jadoogar: new Role({
      ...
      team: Team.MAFIA,
      icon: '🧙',
      descriptionEn: 'Each night blocks one citizen or independent player\'s night action. Target cannot act that night AND the following day. If target has bullets, can still shoot but bullet counts as blank (مشقی). If blocks Gunner → Gunner cannot give bullets. ⚠️ Cannot block same person two nights in a row.',
      descriptionFa: 'هر شب اقدام شبانه یک شهروند یا مستقل را خنثی می‌کند. هدف آن شب و روز بعد نمی‌تواند اقدام کند. اگر هدف تیر داشته باشد، می‌تواند شلیک کند ولی تیرش مشقی (بلنک) حساب می‌شود. اگر تفنگدار بلاک شود → نمی‌تواند تیر بدهد. ⚠️ نمی‌تواند دو شب متوالی یک نفر را بلاک کند.',
    ...
    jack: new Role({
      ...
      icon: '🔪',
      descriptionEn: 'Independent role. Each night curses one person. If the cursed person is killed or voted out (night or day), Jack is also eliminated — this is the ONLY way Jack can die. Jack is immune to all other forms of death (shoot, vote, salakhi, etc.). Cannot curse the same person again unless no other option. If shot in day → survives but curse is permanently locked. If targeted by vote → survives but curse is permanently locked. Wins if all mafia die or if Jack reaches Chaos (3 players).',
      descriptionFa: 'نقش مستقل. هر شب روی یک نفر طلسم می‌گذارد. اگر فرد طلسم‌شده کشته شود (شب یا روز)، جک هم حذف می‌شود — این تنها راه مرگ جک است. جک از تمام انواع دیگر مرگ مصون است (شلیک، رأی، سلاخی و غیره). نمی‌تواند روی یک نفر دوباره طلسم بگذارد مگر گزینه دیگری نباشد. اگر در روز تیر بخورد → زنده می‌ماند ولی طلسم برای همیشه ثابت می‌شود. اگر هدف رأی قرار بگیرد → زنده می‌ماند ولی طلسم ثابت می‌شود. برنده می‌شود اگر تمام مافیا بمیرند یا جک به آشوب (۳ نفر) برسد.',
